...
describe('Commune Management Component', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [RouterTestingModule.withRoutes([{ path: 'commune', component: CommuneComponent }]), HttpClientTestingModule],
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.communes?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to communeService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getCommuneIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getCommuneIdentifier).toHaveBeenCalledWith(entity);
