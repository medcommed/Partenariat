...
describe('Convention Management Component', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [RouterTestingModule.withRoutes([{ path: 'convention', component: ConventionComponent }]), HttpClientTestingModule],
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.conventions?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to conventionService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getConventionIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getConventionIdentifier).toHaveBeenCalledWith(entity);
